...
for env_id in $(seq 6 16); do
    ...
    for seed in $(seq 0 16); do
        uv run python run.py --seed $seed --num-trials 1  --benchmark-id $benchmark --agent-type react --hindsight-type $agent --save-dir results/$benchmark/$model/$agent/$seed --model $model --ruleset-id $env_id
